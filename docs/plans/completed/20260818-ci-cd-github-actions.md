...
- Текущий модуль: `horisont-epub`, Go 1.25+, один `main` package.
...
- [x] Использовать `run_number` исходного CI workflow (`RELEASE_RUN_NUMBER`) как уникальный patch-компонент тега серии `v0.1.x`, исключая гонку между одновременными merge и сохраняя тег при повторном запуске.
...
- [x] Автоматически проверить расчёт разных тегов для разных `RELEASE_RUN_NUMBER` и идемпотентный повтор для того же SHA.
