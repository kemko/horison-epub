...
- [ ] Запускать workflow на каждый push в `master`; через GitHub API требовать связанный merged PR и завершаться ошибкой для прямого push.
- [ ] Использовать `GITHUB_RUN_NUMBER` как уникальный patch-компонент тега серии `v0.1.x`, исключая гонку между одновременными merge.
- [ ] Сделать создание тега идемпотентным: повторный запуск принимает существующий тег только при совпадении SHA.
- [ ] Формировать Markdown-описание из номера, заголовка, тела, автора и ссылки merged PR без интерполяции пользовательского текста в shell.
- [ ] Checkout выполнять с полной историей и тегами; release workflow выдать только `contents: write` и `pull-requests: read`.
- [ ] Запускать закреплённый GoReleaser v2 с подготовленным release-notes файлом и встроенным `GITHUB_TOKEN`.
- [ ] Проверить workflow через actionlint и выполнить snapshot-релиз локально.
- [ ] Автоматически проверить расчёт разных тегов для разных `GITHUB_RUN_NUMBER` и идемпотентный повтор для того же SHA.
- [ ] Запустить `make ci` — все проверки должны пройти до Task 5.
