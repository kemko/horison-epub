...
Для сборки требуется Go 1.26.6.
...
`make vuln` сканирует версию стандартной библиотеки из `go.mod`; для
диагностической проверки другой версии можно задать `VULN_GO_VERSION`, например
`make vuln VULN_GO_VERSION=go1.25.12`.
...
На Windows EPUB временно записывается в системный временный каталог через
`goepub.OsFS`, поскольку `MemoryFS` в текущей версии `go-epub` несовместима с
Windows. Этот fallback можно удалить после перехода на новую версию `go-epub`
с исправленной cross-platform `MemoryFS`.
...
Серия релизов задаётся константой `version_prefix` в начале
`scripts/release-tag.sh`. Для minor-релиза нужно изменить её, например с
`v0.1.` на `v0.2.`, для major-релиза — на `v1.0.`. Первый merge после смены
серии получит patch `1`, если тегов с таким major и minor ещё нет. Последующие
merge продолжат нумерацию от наибольшего тега этой серии.
