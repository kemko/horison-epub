...
Загрузчик по умолчанию не подключается к loopback, private и link-local адресам, включая перенаправления, и не использует proxy из переменных окружения. Каждый HTTP-запрос ограничен 60 секундами, цепочка — 10 перенаправлениями. Для доверенного выпуска во внутренней сети сетевое ограничение можно явно снять флагом `-allow-private-network`.
